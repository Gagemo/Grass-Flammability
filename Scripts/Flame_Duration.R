...
GRASS$Flame_Total = as.numeric(GRASS$Flame_Total)
GRASS$FB = as.numeric(GRASS$FB)

GRASS$Species = factor(GRASS$Species)

GRASS %>% anova_test(Flame_Total ~ Species*FB)

# Check Assumptions #
model  <- lm(Flame_Total ~ FB + Species, data = GRASS)
...
anova_ = GRASS %>% anova_test(Flame_Total ~ FB + Species) %>% 
  add_significance()
anova_
...
# Fit the ANOVA model
anova <- aov(Flame_Total ~ Species + FB, data = GRASS)
